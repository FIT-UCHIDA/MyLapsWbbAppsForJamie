...
class MainWindow(QWidget):
    ...
    def _validate_date_range(self, start_jst: str, end_jst: str) -> bool:
        """期間が最大 _MAX_DATE_RANGE_DAYS 日以内かチェック。Falseの場合はダイアログ表示済み。"""
        fmt = "%Y-%m-%d %H:%M:%S"
        try:
            s = datetime.strptime(start_jst, fmt)
            e = datetime.strptime(end_jst, fmt)
        except ValueError:
            QMessageBox.warning(self, "Date Error", "Invalid date format.")
            return False
        if e <= s:
            QMessageBox.warning(self, "Date Error", "End date must be after start date.")
            return False
        if (e - s).days >= _MAX_DATE_RANGE_DAYS:
            QMessageBox.warning(
                self, "Date Range Error",
                f"Period must be within {_MAX_DATE_RANGE_DAYS} days.\n"
                f"(Specified: {(e - s).days} days)"
            )
            return False
        return True
    ...
    def load_players(self):
        ...
        if not self._validate_date_range(start_jst, end_jst):
            return
