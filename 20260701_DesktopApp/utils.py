...
def split_laps(df, all_data=None, log_file=None):
    ...
    # 次ラップの値を派生列として追加
    result_df["FP_2nd"] = result_df["FP_start"].shift(-1) if "FP_start" in result_df.columns else pd.NaT
    result_df["0m_2nd"] = result_df["0m_start"].shift(-1) if "0m_start" in result_df.columns else pd.NaT
    result_df["BP_2nd"] = result_df["BP"].shift(-1) if "BP" in result_df.columns else pd.NaT

    # Rolling effort 修正: FP_start より前の中間デコーダタイムスタンプをクリア
    # (遷移ラップで前effortの通過記録が混入してマイナス時刻になるのを防ぐ)
